...
export function AwardCard({ award, index, streakCount }: AwardCardProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl bg-card border border-hairline p-4 space-y-3"
    >
      <div className="flex items-center gap-3">
        ...
        {award.statLabel && (
          <div className="shrink-0 px-2.5 py-1 rounded-lg bg-surface-secondary border border-card-border">
            <span className="text-[11px] font-mono font-bold text-muted-foreground">{award.statLabel}</span>
          </div>
        )}
      </div>
