...
export function RoastSection({ groupId, members }: RoastSectionProps) {
  ...
  const getCurrentWeekKey = useRoastStore((s) => s.getCurrentWeekKey);
  ...
  // Auto-generate roast for last week if it doesn't exist
  useEffect(() => {
    const lastWeek = getLastWeekKey();
    if (members.length >= 2 && !hasRecapForWeek(groupId, lastWeek) && !generating && !loading) {
      ...
      <div>
        ...
        {generating && recaps.length === 0 && (
          ...
          >
            ...
            <p className="text-sm text-zinc-400">Cooking up the roast...</p>
            <p className="text-[10px] text-zinc-600 mt-1">Analyzing last week&apos;s data</p>
          </motion.div>
        )}
        ...
        {!generating && recaps.length === 0 && (
          <div className="rounded-2xl bg-white/[0.03] border border-white/[0.05] p-6 text-center">
            ...
            <p className="text-[10px] text-zinc-600 mt-1">
              Start logging drinks and the roast will drop next week
            </p>
          </div>
        )}
