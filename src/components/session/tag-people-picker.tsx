  ...
  return (
    <>
      ...
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] active:bg-white/[0.06] transition-colors text-left"
      >
        <Tag className="w-4 h-4 text-zinc-500 shrink-0" />
        {selectedUsers.length === 0 ? (
          <span className="text-sm text-zinc-600">Tag people</span>
        ) : (
          <div className="flex items-center gap-2 min-w-0">
            <div className="flex -space-x-1.5 shrink-0">
              {selectedUsers.slice(0, 3).map((u) => (
                <Avatar key={u.id} name={u.displayName} size="xs" src={u.avatarUrl} className="ring-1 ring-black" />
              ))}
            </div>
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              style={{ background: 'rgba(20,20,24,0.92)', backdropFilter: 'blur(28px) saturate(180%)', WebkitBackdropFilter: 'blur(28px) saturate(180%)' }}
            >
              {/* Header */}
              <div className="px-5 py-4 flex items-center justify-between border-b border-white/[0.05] shrink-0">
                <h3 className="text-base font-bold">Tag people</h3>
                <button onClick={() => setOpen(false)} className="p-2 -mr-2 rounded-lg hover:bg-white/5 active:bg-white/[0.08]">
              ...
              <div className="px-4 pt-3 pb-2 shrink-0">
                <div className="relative">
                  ...
                  <input
                    ...
                    autoCapitalize="none"
                    className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white/[0.05] border border-white/[0.06] text-[16px] md:text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-accent/40 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain px-2 pb-3">
                {followingUsers.length === 0 ? (
                  <p className="text-sm text-zinc-600 text-center py-10 px-6">
                    Follow people to tag them in your posts.
                  </p>
                ) : filtered.length === 0 ? (
                  <p className="text-sm text-zinc-600 text-center py-10">No matches</p>
                ) : (
                  filtered.map((user) => {
              ...
              {/* Done */}
              <div className="px-4 py-3 border-t border-white/[0.05] shrink-0 safe-bottom">
                <button
