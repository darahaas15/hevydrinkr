...
- **`useThemeStore`** (`src/stores/use-theme-store.ts`) holds `preference: 'light' | 'dark'` (default `'dark'`, the original look) plus `pinPending`, persisted **device-local** via zustand `persist` + `safeJSONStorage` under the key `hd-theme` (version 1). No Supabase/account sync.
  There is deliberately **no System option**: iOS home-screen apps report `prefers-color-scheme` wrong for the first moments after launch and fake a light/dark switch while the app goes to the app switcher, so following the phone opened the app in the wrong theme after a force-quit.
  Don't reintroduce `matchMedia('(prefers-color-scheme ...)')` beyond the two migration uses below (the pin, and the boot script for a saved `'system'` with nothing recorded).
- **Migration from the System era.** Version 0 allowed `'system'` (the old default), and the old controller cached the applied theme under `hd-theme-effective`.
  `migrateThemeSettings()` in `src/lib/theme.ts` keeps a saved Light/Dark choice, gives a fresh device Dark, and keeps a device that followed its phone (saved `'system'`, or saved nothing but has `hd-theme-effective`) on the theme on screen with `pinPending: true`.
  ThemeController then saves the phone's appearance as that device's choice once it has held for `SYSTEM_SETTLE_MS` while the app is visible (an appearance change restarts the wait, hiding cancels it), unless another tab saved a choice first, and deletes `hd-theme-effective`; a force-quit first just retries next launch.
  zustand skips `migrate` when nothing is saved, so the store's initial state runs the same conversion.
  The initial state is also React's hydration snapshot, so its preference is read from the `data-theme` the boot script painted; anything else flashes the default for a frame.
  A custom `merge` drops invalid saved settings, as the boot script does.
- **`ThemeController`** (`src/components/theme/theme-controller.tsx`, mounted in the root layout) writes `data-theme` on `<html>` and syncs `<meta name="theme-color">`. It cross-fades only on a change after mount (a switch in Settings), gated on `prefers-reduced-motion` via the `.theme-transition` class, never on first paint.
- **Anti-FOUC.** A blocking inline `<head>` script (`themeInitScript()` in `src/lib/theme.ts`) sets `data-theme` + `theme-color` before first paint, per `resolveBootTheme()`: the saved choice, else an unpinned device's `hd-theme-effective`, else the phone for a saved `'system'` with nothing recorded, else Dark.
  `src/lib/theme.test.ts` executes the real script and checks it against `resolveBootTheme()` for every stored state; `src/stores/use-theme-store.test.ts` covers the migration; `src/components/theme/theme-controller.test.tsx` (jsdom) hydrates the controller after the real script and covers the pin.
- The Settings **Appearance** control (`src/app/(app)/profile/settings/page.tsx`) is a 2-segment Light/Dark selector wired to `setPreference` with `hapticSelection()`.
